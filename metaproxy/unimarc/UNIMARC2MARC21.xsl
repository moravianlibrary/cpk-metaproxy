...
<xsl:stylesheet version="1.0"
 ...
 <xsl:template name="transform-leader">
    ...
    <xsl:variable name="leader06" select="translate(substring($leader,07,1), 'hlmn', 'amor')"/>
  ...
  <xsl:template name="transform-100">
    ...
    <xsl:variable name="dest18" select="translate(substring($source-105,1,1), 'abcdefghijklmnopy |','abcdefghijklmaop  |')"/>
    <xsl:variable name="dest19" select="translate(substring($source-105,2,1), 'abcdefghijklmnopy |','abcdefghijklmaop  |')"/>
    <xsl:variable name="dest20" select="translate(substring($source-105,3,1), 'abcdefghijklmnopy |','abcdefghijklmaop  |')"/>
    <xsl:variable name="dest21" select="translate(substring($source-105,4,1), 'abcdefghijklmnopy |','abcdefghijklmaop  |')"/>
	...
	<xsl:value-of select="translate(substring($source-106,1,1), 'abcdfghiryz|','abcdfghir z|')"/>
    </xsl:when>
    <xsl:otherwise>
        <xsl:value-of select="'|'"/>
    </xsl:otherwise>
    ...
    <xsl:variable name="dest24" select="translate(substring($source-105,5,1), ' abcdefghijklmnopqrz|',' bciaderysp   l t n |')"/>
    <xsl:variable name="dest25" select="translate(substring($source-105,6,1), ' abcdefghijklmnopqrz|',' bciaderysp   l t n |')"/>
    <xsl:variable name="dest26" select="translate(substring($source-105,7,1), ' abcdefghijklmnopqrz|',' bciaderysp   l t n |')"/>
    <xsl:variable name="dest27" select="translate(substring($source-105,8,1), ' abcdefghijklmnopqrz|',' bciaderysp   l t n |')"/>
    <xsl:variable name="dest28" select="translate(substring($source,21,1), 'abcdefghy|', 'fsllcizo||')"/>
    <xsl:variable name="dest29-32" select="'||||'"/>
    <xsl:variable name="dest33" select="translate(substring($source-105,12,1), 'abcdefghyz|','1||||1|||||')"/>
    <xsl:variable name="dest34" select="translate(substring($source-105,13,1), 'abcdy|','abcd |')"/>
    ...
    <marc:controlfield tag="008">
      <xsl:value-of select="concat($dest00-05, $dest06, $dest07-14, $dest15-17, $dest18, $dest19, $dest20, $dest21, $dest22, $dest23, $dest24, $dest25, $dest26, $dest27, $dest28, $dest29-32, $dest33, $dest34, $dest35-37, $dest38, $dest39-40)"/>
    </marc:controlfield>
  </xsl:template>
  ...
  <xsl:template name="transform-subfields">
    ...
    <xsl:for-each select="marc:subfield[contains($srcCodes, @code)]">
      <marc:subfield code="{translate(@code, $srcCodes, $dstCodes)}">
        <xsl:value-of select="text()"/>
      </marc:subfield>
    </xsl:for-each>
  </xsl:template>
